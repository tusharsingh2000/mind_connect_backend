...
const login = async (data) => {
    ...
    admin = await Model.admin.findOne({ _id: mongoose.Types.ObjectId(admin._id) }).populate("subAdminId").lean();
...
async function verifyEmail(data, user, removeOtp = true) {
    ...
    let admin = await Model.admin.findOne({ email: data.email.toLowerCase() }).lean();
...
async function updateProfile(req) {
    ...
    let updatedUser = await Model.admin.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.admin._id) }, data, { new: true }).lean();
    return updatedUser;
}
...
async function addSubAdmin(data) {
    let subadmin = await Model.admin.findOne({ email: data.body.email, isDeleted: false });
...
async function getSubAdmin(data) {
    ...
    let subAdmin;
    if (data.params.id) {
        subAdmin = await Model.admin.findOne({ _id: ObjectId(data.params.id), ...qry }).populate("subAdminId");
    } else {
        subAdmin = await Model.admin.find({ ...qry }).populate("subAdminId").skip(skip).limit(limit);
    }
    return subAdmin;
}
async function updateSubAdmin(data) {
    let admin = await Model.admin.findOne({ _id: data.params.id, isDeleted: false });
    if (!admin) throw process.lang.SUBADMIN_NOT_FOUND;

    let subAdmin = await Model.subAdmin.findOne({ _id: admin.subAdminId, isDeleted: false });
    if (!subAdmin) throw process.lang.SUBADMIN_NOT_FOUND;

    if (data.password) {
        data.password = await utility.hashPasswordUsingBcrypt(data.body.password);
    }
    await Model.subAdmin.findByIdAndUpdate({ _id: admin.subAdminId }, data.body, { new: true });
    await Model.admin.findByIdAndUpdate({ _id: data.params.id }, data.body, { new: true });
    subAdmin = await Model.admin.findOne({ _id: ObjectId(data.params.id), isDeleted: false }).populate("subAdminId");
    return subAdmin;
}
async function deleteSubAdmin(data) {
    ...
    let subAdmin = await Model.subAdmin.findOne({ _id: admin.subAdminId, isDeleted: false });
    if (!subAdmin) throw process.lang.SUBADMIN_NOT_FOUND;
    await Model.admin.findByIdAndUpdate({ _id: data.params.id, isDeleted: false }, { isDeleted: true }, { new: true });
    await Model.subAdmin.findByIdAndUpdate({ _id: admin.subAdminId, isDeleted: false }, { isDeleted: true }, { new: true });
    return {};
}
...
async function getDashboard(data) {
    ...
    if (data.query.personType == "daily") {
        var su = 0,
            ...
            fr = 0,
            sa = 0,
            users = await Model.user.aggregate([{ $match: { createdAt: { $gte: StartofWeek, $lte: EndofWeek }, isDeleted: false } }]);
        users.map((val) => {
        ...
        detail.persondata = { sun: su, mon: mo, tue: tu, wed: we, thr: th, fri: fr, sat: sa };
    } else if (data.query.personType == "weekly") {
        var w1 = 0,
            w2 = 0,
            w3 = 0,
            w4 = 0;
        users = await Model.user.aggregate([{ $match: { createdAt: { $gte: startOFMonth, $lte: endOFMonth }, isDeleted: false } }]);
        users.map((val) => {
            // console.log(val);
        ...
        var jan = 0,
            ...
            nov = 0,
            dec = 0;
        users = await Model.user.aggregate([{ $match: { createdAt: { $gte: startOFYear, $lte: endOFYear }, isDeleted: false } }]);
        users.map((val) => {
        ...
        users = await Model.user.aggregate([{ $match: { isDeleted: false } },
    ...
    if (data.query.newPersonType == "daily") {
        ...
        detail.newPersondata = { sun: su, mon: mo, tue: tu, wed: we, thr: th, fri: fr, sat: sa };
...
async function getCategory(req) {
    ...
    let category;
    if (req.params.id) {
        category = await Model.category.findOne({ _id: ObjectId(req.params.id), ...qry });
    } else {
        let pipeline = [];
...
async function updateCategory(req) {
    let category = await Model.category.findOne({ _id: req.params.id, isDeleted: false });
    if (!category) throw process.lang.INVALID_ID;

    return await Model.category.findByIdAndUpdate({ _id: req.params.id }, req.body, { new: true });
}
...
async function deleteCategory(req) {
    ...
    await Model.category.findByIdAndUpdate({ _id: category._id, isDeleted: false }, { isDeleted: true }, { new: true });
    return {};
}
