...
async function createUser(data) {
    ...
    if (utility.isEmail(data.key)) {
        data.email = data.key;
        user = await Model.user.findOne({ email: data.email, isEmailVerify: true, isDeleted: false, role: data.role });
        if (!user) {
            user = await Model.user.findOne({ email: data.email, isDeleted: false, role: data.role });
            if (user) {
        ...
        if (!user) {
            user = await Model.user.findOne({ phone: data.phone, countryCode: data.countryCode, isDeleted: false, role: data.role });
            if (user) {
...
async function logout(req) {
    return await Model.user.findOneAndUpdate({ _id: ObjectId(req.user._id), isDeleted: false },
...
async function verifyOTP(data) {
    ...
    if (utility.isEmail(data.key)) {
        ...
        data.phone = data.key;
        user = await Model.user.findOne({ countryCode: data.countryCode, phone: data.phone, isDeleted: false, role: data.role });
    ...
    user.token = await utility.jwtSign({ _id: user._id, role: user.role, jti: jti });
    user.type = "Bearer";
...
async function _doLogin(user, data) {
    ...
    let jti = utility.generateRandomString(20);
    setObj.jti = jti;
    user = await Model.user.findByIdAndUpdate({ _id: user._id }, { $set: setObj }).lean();
...
async function forgotpassword(data) {
    if ((data.key && utility.isEmail(data.key)) || data.email) {
        ...
        let user = await Model.user.findOne({ email: email.toLowerCase(), isDeleted: false });
        if (!user) {
...
async function userNotification(req) {
    ...
    let old = await Model.notification.find({ isDeleted: false, isRead: true, userId: ObjectId(req.user._id) }).sort({ createdAt: -1 });
    let newNoti = await Model.notification.find({
        isDeleted: false, isRead: false, userId: ObjectId(req.user._id)
    }).populate("userId reactId").sort({ createdAt: -1 }).skip(skip).limit(limit);
    let count = await Model.notification.countDocuments({ isDeleted: false, isRead: false, userId: ObjectId(req.user._id) });

    await Model.notification.updateMany({ isDeleted: false, isRead: false, userId: ObjectId(req.user._id) },
        { $set: { isRead: true } });
    return { old: old, new: newNoti, count: count };
}
async function userClearNotification(req) {
    let notification = await Model.notification.updateMany({ userId: req.user._id, isDeleted: false }, { $set: { isDeleted: true } });
    return notification;
}
...
async function getExperience(req) {
    ...
    let experience;
    if (req.params.id) {
        experience = await Model.experience.findOne({ _id: ObjectId(req.params.id), ...qry }).populate("categoryId").
