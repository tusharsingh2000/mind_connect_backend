...
async function dashboard(req) {
    ...
    let setting = await Model.setting
...
async function addEducation(req) {
    ...
    if (req.body.profileCompleteAt) {
        await Model.user.findOneAndUpdate(
            { _id: ObjectId(req.user._id) },
            { $set: { profileCompleteAt: req.body.profileCompleteAt } }
        );
    }
...
async function addExperience(req) {
    ...
    if (req.body.profileCompleteAt) {
        await Model.user.findOneAndUpdate({ _id: ObjectId(req.user._id) },
            { $set: { profileCompleteAt: req.body.profileCompleteAt } }
        );
    }
...
async function updateExperience(req) {
    let experience = await Model.experience.findOne({
        _id: req.params.id,
        isDeleted: false,
        userId: ObjectId(req.user._id)
    })
    ...
    if (req.body.categoryId) {
        await Model.user.findByIdAndUpdate({ _id: req.user._id, isDeleted: false },
            { $set: { categoryId: req.body.categoryId } },
            { new: true }
        );
    }
...
async function deleteExperience(req) {
    let experience = await Model.experience.findOne({
        _id: req.params.id,
        isDeleted: false, userId: ObjectId(req.user._id)
    })
        .select("-createdAt -updatedAt");
    if (!experience) throw process.lang.INVALID_ID;

    await Model.experience.findByIdAndUpdate({ _id: experience._id, isDeleted: false },
...
async function addAddress(req) {
    req.body.userId = req.user._id;
    if (req.body.profileCompleteAt) {
        await Model.user.findOneAndUpdate({ _id: req.user._id },
            { $set: { profileCompleteAt: req.body.profileCompleteAt } }
        );
    }
...
async function getAddress(req) {
    ...
    let address;
    if (req.params.id) {
        address = await Model.address.findOne({ _id: ObjectId(req.params.id), ...qry })
            .select("-createdAt -updatedAt");
    } else {
        let pipeline = [];
        pipeline.push({
...
async function addDocument(req) {
    ...
    if (req.body.isProfileComplete) {
        await Model.user.findOneAndUpdate(
            { _id: ObjectId(req.user._id) }, { $set: { isProfileComplete: req.body.isProfileComplete } }
        );
    }
...
async function updateDocument(req) {
    ...
    if (req.body.isProfileComplete) {
        await Model.user.findOneAndUpdate({ _id: ObjectId(req.user._id) },
            { $set: { isProfileComplete: req.body.isProfileComplete } }
        );
    }
